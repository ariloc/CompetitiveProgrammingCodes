...
int tarjan (int st) { // SCC con Tarjan
    ...
    if (mini >= stTime[st])
        while (not pila.empty()) {
            ...
            // si est�n en la misma componente los opuestos chau, as� que
            // no me importa que meta esto de la pila y lo cuente al rev�s
            // y directamente me guardo posiciones hipot�ticas as� puedo comparar
            // de a pares
            posTopSort[e] = pos++;
...
int main() {
    ...
    forn(i,n) { // duplicar es +, de lo contrario -
        ...
        x1 *= 2; x2 *= 2; // duplico as� tengo mis opuestos, adyacentes en el array
    ...
    for (int i = 0; i < 2*m; i += 2) { // comparo adyacentes, salto de a 2
        if (col[i] == col[i+1]) {posib = false; break;} // y si est�n en la misma SCC, chau
        // si lo puse primero, est� debajo en el topSort, y a es verdadero si est� despu�s de �a
