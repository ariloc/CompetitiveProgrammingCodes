...
int main() {
    ...
    forn(_,t) {
        ...
        forn(i,q) {
            int l,r; cin >> l >> r;
            int cot = (r-l+1)/3;
            int x1 = cot+1, x2 = cot*2+1, x3 = r-l+1;
            if (r-l+1 < 3) x1 = 1, x2 = 2;
            ii c1 = query(pref[r],l > 1 ? pref[l-1] : DUM,0,N,x1);
            ii c2 = query(pref[r],l > 1 ? pref[l-1] : DUM,0,N,x2);
            ii c3 = query(pref[r],l > 1 ? pref[l-1] : DUM,0,N,x3);

            int cnt = 0;
            if (c1.snd > cot) cout << nums[c1.fst] << ' ', ++cnt;
            if (c2.snd > cot && c1.fst != c2.fst) cout << nums[c2.fst] << ' ', ++cnt;
            if (c3.snd > cot && c3.fst != c2.fst && c3.fst != c1.fst) cout << nums[c3.fst] << ' ', ++cnt;
            if (cnt == 0) cout << "-1";
            cout << '\n';
        }
    }
