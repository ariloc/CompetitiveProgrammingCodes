...
int main() {
    ...
    dforn(i,sz(we)) {
        ...
        forn(j,sz(byw[i])) {
            ii cur = byw[i][j];
            if (UF.find(cur.fst).fst == UF.find(cur.snd).fst && UF.find(cur.fst).snd != UF.find(cur.snd).snd)
                continue;
            safe.pb(cur);
        }
        ...
        auto go = [&](vector<ii> &vec) {
            vector<ii> p1,p2;
            for (auto &k : vec) if (UF.find(k.fst).snd) p2.pb(k); else p1.pb(k);
        ...
        auto goconn = [&](int j, int l, int r) {
            ...
            forsn(k,l,r) {
                ...
                if (UF.find(cur.fst).snd == UF.find(cur.snd).snd) eq.pb(cur); else neq.pb(cur);
            }
            ...
            forn(k,sz(conn2[j])) {
                if (k && conn2[j][k-1].fst != conn2[j][k].fst) {
                    forsn(l,idx,k) {
                        ...
                        if (UF.find(cur.fst).snd) inc(cur,1);
                        else inc(cur,0);
                    }
                    idx = k;
                }
                ii cur = conn2[j][k].snd;
                if (UF.find(cur.fst).snd) check(cur,0);
                else check(cur,1);
            }
        }
