...
bool isPosib (int mid) {
    ...
    int minP = 0, acc = 0;
    forn(i,n) {
        if (i-k >= 0) minP = min(minP,dp[i-k]); // consideremos el prefijo más chico tal que al menos me deja un subsegm de tamaño k
        acc += (arr[i] < mid ? -1 : 1); dp[i] = acc;
        if (i+1 >= k && acc-minP > 0) return true;
    }
    return false;
}
