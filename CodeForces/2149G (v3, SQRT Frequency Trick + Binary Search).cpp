...
int arr[MAXN], ap[MAXN], fuap[MAXN];
vi elap[MAXN], rtas[MAXN];
...
int main() {
    ...
    forn(_,t) {
        ...
        forn(i,n) ap[i] = 0, fuap[i] = 0, elap[i].clear();
        ...
        for (auto &i : quer) {
            ...
            if (cot <= b) {
                ...
                dforn(j,pars.size()) {
                    if (pars[j].fst <= cot) break;
                    int lind = lower_bound(all(elap[pars[j].snd]), l)-elap[pars[j].snd].begin();
                    if ((int)elap[pars[j].snd].size() - lind > cot)
                        rtas[i.snd].pb(pars[j].snd);
                }
            }
            sort(all(rtas[i.snd]));
