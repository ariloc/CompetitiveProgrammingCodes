...
void compute_45deg(pt a1, pt a2, pt b1, pt b2) {
    if ((a1.x == a2.x) == (b1.x == b2.x))
        return;
    if (b1.y == b2.y) swap(a1,b1), swap(a2,b2);

    // si van "en mismo sentido", calcular el 45° no me afecta en nada, porque calculo igual en los extremos
    // si van en distinto me conviene
    // además, si uno está "en medio" del trayecto del otro así:
    //    |
    //    |
    //
    // ------
    // ... independientemente del sentido gana algún múltiplo de 45°

    
    pt d1 = b1-a1;
    ld off1 = abs(d1.x-d1.y)/2;
    pt ex1 = addv(a1,off1,a1,a2), ex2 = addv(b1,off1,b1,b2);
    if (ex1.x-min(a1.x,a2.x) > -EPS && max(a1.x,a2.x)-ex1.x > -EPS && ex1.y-min(a1.y,a2.y) > -EPS && max(a1.y,a2.y)-ex1.y > -EPS
            && ex2.x-min(b1.x,b2.x) > -EPS && max(b1.x,b2.x)-ex2.x > -EPS && ex2.y-min(b1.y,b2.y) > -EPS && max(b1.y,b2.y)-ex2.y > -EPS) {
    ...
    if (b1.x-min(a1.x,a2.x) > EPS && max(a1.x,a2.x)-b1.x > EPS) { // _|_
        ld off = b1.x-min(a1.x,a2.x);
        pt ma = addv(a1,off,a1,a2), mb = addv(b1,off,b1,b2);
        compute_45deg(a1,ma,b1,mb);
        compute_45deg(ma,a2,mb,b2);
    }

    if (a1.y-min(b1.y,b2.y) > EPS && max(b1.y,b2.y)-a1.y > EPS) { // -|
        ld off = a1.y-min(b1.y,b2.y);
        pt ma = addv(a1,off,a1,a2), mb = addv(b1,off,b1,b2);
        compute_45deg(a1,ma,b1,mb);
        compute_45deg(ma,a2,mb,b2);
    }
